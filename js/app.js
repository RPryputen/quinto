(() => {
    ...
    (() => {
        ...
        da.init();
        function onSearchToogle(toogleSelector, searchSelector) {
            const toogle = document.querySelector(toogleSelector);
            const search = document.querySelector(searchSelector);
            const searchData = `${searchSelector.slice(1, -1)}`;
            const showSearch = () => search.setAttribute(searchData, "true");
            const hideSearch = () => search.setAttribute(searchData, "false");
            const toggleMenu = () => {
                const atrr = search.getAttribute(searchData);
                if ("false" === atrr) showSearch(); else hideSearch();
            };
            if (toogle) {
                ...
                document.addEventListener("click", (e => {
                    const target = e.target;
                    const searchEvent = target == search || search.contains(target);
                    const toogleEvent = target == toogle || toogle.contains(target);
                    const atrr = search.getAttribute(searchData);
                    if (!searchEvent && !toogleEvent && "true" === atrr) toggleMenu();
                }));
            }
        }
        onSearchToogle("[data-search-toogle]", "[data-search]");
        function onToogle(toogleSelector) {
            const toogle = document.querySelectorAll(toogleSelector);
            const toogleData = `${toogleSelector.slice(1, -1)}`;
            toogle.forEach((item => {
                item.addEventListener("click", (() => {
                    if ("false" === item.getAttribute(toogleData)) item.setAttribute(toogleData, "true"); else item.setAttribute(toogleData, "false");
                }));
            }));
        }
        onToogle("[data-toogle]");
        class StarRating {
            constructor(nameSelector) {
                this.stars = document.querySelectorAll(nameSelector);
                this.starsData = `${nameSelector.slice(1, -1)}`;
                this.stars.forEach(((item, i) => {
                    item.setAttribute("data-count", i);
                    item.addEventListener("click", this.activeStarListener.bind(this));
                }));
            }
            activeStarListener(e) {
                this.starsElementChange(e.target);
            }
            disabledStarListener() {
                this.starsElementChange(null);
            }
            starsElementChange(el) {
                this.stars.forEach((item => {
                    const attributeСomparison = item.getAttribute("data-count") > el.getAttribute("data-count");
                    if (null == el || attributeСomparison) item.setAttribute(this.starsData, false); else item.setAttribute(this.starsData, true);
                }));
            }
        }
